...
from app.controllers import create_user, get_all_users, get_users_by_role
...
async def get_own_students(
    user: User = Security(get_current_user, scopes=[Role.teacher]),
    db_session: Session = Depends(get_db),
) -> list[StudentProfileSchema]:
    return parse_obj_as(list[StudentProfileSchema], user.teacher.students)
